...
const Page: React.FC = () => {
  ...
  if (selectedArticle) {
    return (
      <>
        ...
        <main className="[background:radial-gradient(ellipse_at_top_right,rgba(168,85,247,0.05)_0%,transparent_50%), radial-gradient(ellipse_at_bottom_left,rgba(255,215,0,0.04)_0%,transparent_50%)] min-h-screen w-full overflow-x-hidden bg-black pt-[8vh] text-white">
          ...
          <div className="mx-auto max-w-225 px-8 pb-16">
            <button
              ...
              className="cursor-pointer rounded-lg border-0 bg-[linear-gradient(135deg,#ffd700_0%,#ffed4e_50%,#ffd700_100%)] px-8 py-4 text-base font-semibold text-black transition-transform duration-200 ease-out hover:-translate-y-0.5"
            >
  ...
  return (
    <>
      <NavBar showOnScroll={false} variant={1} />
      <main className="[background:radial-gradient(ellipse_at_top_right,rgba(168,85,247,0.05)_0%,transparent_50%), radial-gradient(ellipse_at_bottom_left,rgba(255,215,0,0.04)_0%,transparent_50%)] min-h-screen w-full overflow-x-hidden bg-black bg-linear-to-b from-blue-800 to-black to-10% pb-15 font-sans leading-relaxed text-white">
        <section className="mx-auto grid max-w-400 grid-cols-1 gap-12 px-12 pt-25 pb-10 max-md:px-6 max-md:pt-8">
          <Article
        ...
        {hackathonArticles.length > 0 && (
          <>
            <div className="mx-auto max-w-400 px-12 pt-8 pb-8 max-md:px-6">
              <h2 className="to-gold bg-linear-to-r from-white bg-clip-text text-[1.75rem] font-bold tracking-tight text-transparent max-md:text-2xl">
                Hackathon
              </h2>
            </div>

            <section className="mx-auto max-w-400 overflow-visible px-12 pb-12 max-md:px-6">
              <div className="-my-8 flex snap-x snap-mandatory scroll-pl-12 gap-8 overflow-x-auto overflow-y-visible py-8 pb-12 [scrollbar-color:#ffd700_#0d0d0d] [scrollbar-width:thin] max-md:scroll-pl-6 [&::-webkit-scrollbar]:h-2.5 ...">
                {hackathonArticles.map((article) => (
                  <div
                    key={article.id}
                    className="w-65 max-w-65 min-w-65 shrink-0 snap-start max-md:w-60 max-md:max-w-60 max-md:min-w-60"
                  >
                    <Article
                      data={article}
                      variant="card"
        ...
        <div className="mx-auto max-w-400 px-12 pt-12 pb-8 max-md:px-6">
          <h2 className="to-gold bg-linear-to-r from-white bg-clip-text text-[1.75rem] font-bold tracking-tight text-transparent max-md:text-2xl">
            Latest Stories
          </h2>
        </div>
