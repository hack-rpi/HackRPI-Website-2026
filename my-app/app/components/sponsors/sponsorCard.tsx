...
export default function SponsorCard({ name, tier, image }: any) {
  ...
  if (tier === "obsidian") {
    bg_color = "bg-slate-900/30 bg-gradient-to-br from-white/10 to-transparent border border-white/20";
    text_color = "text-purple-300";
    shadowColor = "147,51,234";
  } else if (tier === "gold") {
    bg_color = "bg-yellow-400/30 bg-gradient-to-br from-white/10 to-transparent border border-white/20";
    text_color = "text-yellow-100";
    shadowColor = "234,179,8";
  } else if (tier === "silver") {
    bg_color = "bg-slate-300/30 bg-gradient-to-br from-white/10 to-transparent border border-white/20";
    text_color = "text-white";
    shadowColor = "59,130,246";
  } else if (tier === "bronze") {
    bg_color = "bg-orange-700/30 bg-gradient-to-br from-white/10 to-transparent border border-white/20";
    text_color = "text-orange-100";
    shadowColor = "239,68,68";
  } else {
    bg_color = "bg-white/30 bg-gradient-to-br from-white/10 to-transparent border border-white/20";
    ...
    <div className="m-6 flex flex-col items-center group">
      ...
      >
        <div className="flex flex-col items-center justify-center gap-2">
          {image && <img src={image} alt={name} className="absolute inset-0 w-full h-full object-contain" />}
        </div>
      </div>

      <span className={style2}>{tier}</span>
    </div>
  );
}
