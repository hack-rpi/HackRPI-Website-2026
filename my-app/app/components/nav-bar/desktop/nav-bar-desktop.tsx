...
function NavGroupComponent({ name, links }: { name: string; links: lin[] }) {
  ...
  return (
    <div className="dropdown-hover dropdown">
      ...
      <ul
        tabIndex={-1}
        className="dropdown-content menu w-52 bg-linear-to-b from-white/50 to-sky-500/90 p-2 backdrop-blur-sm"
      >
        {links.map((link) => (
...
export default function DesktopNavBar({ links }: { links: NavGroup[] }) {
  return (
    <>
      ...
      <div className="h-16 w-full bg-linear-to-b from-white/90 to-sky-500/50 text-slate-800 backdrop-blur-sm">
        <div className="z-50 flex h-full w-[95%] items-center justify-center" role="navigation">
